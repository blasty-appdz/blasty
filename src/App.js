...
function ProDashboard({ user, isAr }) {
  ...
  const stats=[
    [String(rdvs.length),  isAr?"مواعيد اليوم":"RDV total",       C.blue],
    [String(confirmed),    isAr?"مؤكدة":"Confirmés",               C.success],
    [String(pending),      isAr?"في الانتظار":"En attente",        C.warn],
    ["⭐",                 isAr?"تقييمك":"Votre note",             "#9B59B6"],
  ];
          ...
          <Card key={i} style={{ padding:"14px 16px" }}>
            <div style={{ display:"flex", alignItems:"center", gap:12 }}>
              ...
              <div style={{ flex:1 }}>
                ...
                <div style={{ fontSize:13, color:C.muted, marginTop:2 }}>📞 {r.client_phone}</div>
              </div>
              <StatusBadge status={r.status} isAr={isAr} />
            </div>
          </Card>
